...
export async function POST(
  ...
  try {
    ...
    // 5. Retrieve top 3 AML knowledge entries
    const queryText = riskSignals.join(' ') + ' ' + (caseObj.crime_category || '') + ' cryptocurrency risk';
    const sources = retrieveRelevantKnowledge(queryText, {
      risk_signals: riskContextSignals(caseContext),
      network: caseObj.chain,
      wallet: caseObj.wallet_address
    });
...
${doc.section ? `Section: ${doc.section}` : ''} ${doc.page ? `Page: ${doc.page}` : ''}`
    ...
    // 6. Call Groq
    const analysis = await callGroqLLM(SYSTEM_PROMPT, caseContext, undefined, ragContext);
...
function riskContextSignals(context: any): string[] {
  const list: string[] = [...context.risk_signals];
  if (context.transactions && context.transactions.length > 20) {
    list.push("High Activity Wallet");
  }
  return list;
}
